...
- **`[permalinks]`** — Three sections get an explicit `:slug` permalink so their URLs look like `/notices/foo/` instead of `/notices/2026-06-21-foo/`. The `:slug` is the explicit `slug:` field that every dated post pins in its frontmatter (the dated filename is only for editor sort order); a post that omits `slug:` would fall back to a title-derived slug, which since #56 fails the build instead. `governance` is intentionally omitted because those pages aren't dated.
